...
--No permite borrar debido a las relaciones entre tablas establecidas previamente
...
--No afecta debido a que en la tabla empleados, no hay empleados registrados en esa sucursal

--------CONSULTAS SQL------------
--¿Cuál es la cantidad total de registros únicos en la tabla de órdenes?

SELECT COUNT(OrdenID) AS TotalOrdenes 
    FROM Ordenes;

--¿Cuántos empleados existen en cada departamento?

SELECT COUNT(EmpleadoID) Nro_Empleados, Departamento
    FROM Empleados
...
/*Pregunta: ¿Cuáles son las sucursales con un promedio de Facturación/Ingresos superior a 1000.00 y que minimizan sus costos en base al promedio 
de kilómetros recorridos de todas de sus entregas gestionadas? Para un mejor relevamiento, ordene el listado por el Promedio Km Recorridos.*/

SELECT SucursalID,
    CAST(AVG(TotalCompra) AS DECIMAL(10,2))AS PromedioCompra,
    CAST(AVG(KilometrosRecorrer) AS DECIMAL(10,2)) AS KmRecorridos
FROM Ordenes
GROUP BY SucursalID
HAVING AVG(TotalCompra) > 1000.00
ORDER BY KmRecorridos;